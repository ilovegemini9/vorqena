...
const intentMeta = {
  fix: {
    title: "Fix Everyday Problems | Vorqena",
    description: "Practical troubleshooting guides for everyday problems, with safe checks, likely causes, and next steps.",
    heading: "Find the likely cause. Take the next step.",
  },
  calculate: {
    title: "Calculators & Everyday Math | Vorqena",
    description: "Focused calculators for percentages, dates, money, loans, and everyday decisions.",
    heading: "Put the numbers in. Get a useful answer.",
  },
  decide: {
    title: "Decision Guides | Vorqena",
    description: "Practical decision guides that turn uncertain everyday choices into clear tradeoffs and next actions.",
    heading: "Know what makes sense before you act.",
  },
  when: {
    title: "Dates, Deadlines & Timing Tools | Vorqena",
    description: "Find dates, deadlines, durations, and timing answers with focused everyday tools.",
    heading: "Get the date, deadline, or timing.",
  },
  cost: {
    title: "Cost Estimators & Guides | Vorqena",
    description: "Estimate everyday costs with transparent assumptions and practical next steps.",
    heading: "Estimate the cost before you commit.",
  },
} as const;
...
function setMeta(shell: string, title: string, description: string, canonical: string, robots = "index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1") {
  let html = shell
    ...
    .replace(/<meta name="googlebot" content="[^"]*"\s*\/>/, `<meta name="googlebot" content="${robots}" />`)
    .replace(/<link rel="canonical" href="[^"]*"\s*\/>/, `<link rel="canonical" href="${canonical}" />`)
    .replace(/<script type="application\/ld\+json">.*?<\/script>\s*/gs, "");
  return html;
}

function withRootContent(shell: string, content: string) {
...
function knowledgeContent(item: (typeof knowledge)[number]) {
  const list = (title: string, values?: string[]) => values?.length
    ? `<section><h2>${title}</h2><ul>${values.map(value => `<li>${escapeHtml(value)}</li>`).join("")}</ul></section>`
    : "";
  const sources = item.sources.length
    ? `<section><h2>Sources</h2><ul>${item.sources.map(source => `<li><a href="${escapeHtml(source.url)}" rel="nofollow">${escapeHtml(source.label)}</a></li>`).join("")}</ul></section>`
    : "";
  const related = item.related.length
    ? `<section><h2>Related</h2><ul>${item.related.map(url => `<li><a href="${url}">${escapeHtml(url.split("/").pop()?.replaceAll("-", " ") ?? url)}</a></li>`).join("")}</ul></section>`
    : "";
...
function hubContent(intent: Intent) {
  ...
  const itemLinks = items.map(item => `<li><a href="${item.slug}"><strong>${escapeHtml(item.title)}</strong> — ${escapeHtml(item.seo.description)}</a></li>`).join("");
...
const homepageJsonLd = {
  ...
  name: "Vorqena",
  url: `${SITE}/`,
  description: "Everyday answers, tools, and decisions.",
  publisher: { "@type": "Organization", name: "Vorqena", url: SITE },
};
...
for (const intent of Object.keys(intentMeta) as Intent[]) {
  const meta = intentMeta[intent];
  const route = `/${intent}`;
  const html = withRootContent(setMeta(shell, meta.title, meta.description, `${SITE}${route}`), hubContent(intent));
...
for (const item of knowledge) {
  ...
  const html = withRootContent(setMeta(shell, item.seo.title, item.seo.description, `${SITE}${route}`), knowledgeContent(item));
  writeRoute(route, html);
  if (item.seo.indexable) indexableRoutes.add(route);
}

for (const tool of tools) {
  const route = `/tool/${tool.slug}`;
  const duplicate = duplicateToolToKnowledge[tool.slug];
  const isIndexable = !duplicate;
  const title = `${tool.title} | Vorqena`;
  const description = tool.description;
  const base = setMeta(shell, title, description, duplicate ? `${SITE}${duplicate}` : `${SITE}${route}`, isIndexable ? "index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1" : "noindex,follow");
  writeRoute(route, withRootContent(base, toolContent(tool)));
  if (isIndexable) indexableRoutes.add(route);
}

const searchHtml = withRootContent(setMeta(shell, "Search Vorqena", "Search Vorqena for everyday answers and tools.", `${SITE}/search`, "noindex,follow"), `<main class="page-shell"><section class="content-wrap"><h1>Search Vorqena</h1><p>Find a direct answer or the right tool.</p></section></main>`);
writeRoute("/search", searchHtml);
...
for (const [route, title, description] of trustPages) {
  const html = withRootContent(setMeta(shell, `${title} | Vorqena`, description, `${SITE}${route}`), `<main class="page-shell"><section class="content-wrap"><span class="eyebrow">Vorqena</span><h1>${escapeHtml(title)}</h1><p>${escapeHtml(description)}</p><p><a href="/">Return to Vorqena</a></p></section></main>`);
  writeRoute(route, html);
  indexableRoutes.add(route);
}
