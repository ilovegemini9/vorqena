...
for (const tool of tools) { const route = `/tool/${tool.slug}`; const duplicate = duplicateToolToKnowledge[tool.slug]; const isIndexable = !duplicate; const title = `${tool.title} | Vorqena`; const base = setMeta(shell, title, tool.description, duplicate ? `${SITE}${duplicate}` : `${SITE}${route}`, isIndexable ? "index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1" : "noindex,follow"); writeRoute(route, withRootContent(base, toolContent(tool))); if (isIndexable) indexableRoutes.add(route); }
