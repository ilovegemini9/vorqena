...
export default function ToolPage() {
  ...
  const result = useMemo(() => {
    ...
    switch (slug) {
      ...
      case "loan-payment": {
        const r = y / 100 / 12, n = Math.max(1, z * 12);
        return r ? `$${(x * r * Math.pow(1 + r, n) / (Math.pow(1 + r, n) - 1)).toFixed(2)} / month` : `$${(x / n).toFixed(2)} / month`;
      }
  ...
  return <main className="page-shell">
    ...
    <section className="tool-layout"><div className="tool-copy"><Link href={`/${tool.intent}`} className="back-link"><ArrowLeft size={15}/> {tool.intent}</Link><span className="eyebrow">Vorqena tool</span><h1>{tool.title}</h1><p>{tool.description}</p><div className="answer-card"><span className="answer-label">Your answer</span><strong>{result}</strong><span>Calculated from the values you entered. Review assumptions before making a consequential decision.</span></div></div>
      {calculator ? <div className="tool-card"><div className="tool-card-head"><Calculator/><h2>Calculator</h2></div>
        {dateTool ? <>
          {slug === "age-calculator" && <Field label="Date of birth" type="date" value={dateA} setValue={setDateA}/>} 
          {slug === "days-between-dates" && <><Field label="Start date" type="date" value={dateA} setValue={setDateA}/><Field label="End date" type="date" value={dateB} setValue={setDateB}/></>}
          {slug === "date-after-days" && <><Field label="Starting date" type="date" value={dateA} setValue={setDateA}/><Field label="Number of days" value={a} setValue={setA}/></>}
        </> : <>
          <Field label={slug === "tip-calculator" ? "Bill amount" : slug === "fuel-cost" ? "Distance" : slug === "electricity-cost" ? "Power" : slug === "break-even" ? "Fixed costs" : "Value"} value={a} setValue={setA} prefix={slug === "tip-calculator" ? "$" : undefined} suffix={slug === "fuel-cost" ? "mi" : slug === "electricity-cost" ? "W" : slug === "break-even" ? "$" : undefined}/>
          <Field label={slug === "tip-calculator" ? "Tip rate" : slug === "fuel-cost" ? "Fuel efficiency" : slug === "electricity-cost" ? "Hours" : slug === "break-even" ? "Price / unit" : "Rate / percentage"} value={b} setValue={setB} suffix={slug === "fuel-cost" ? "mi/gal" : slug === "electricity-cost" ? "hr" : slug === "break-even" ? "$" : "%"}/>
          <Field label={slug === "tip-calculator" ? "People" : slug === "fuel-cost" ? "Fuel price" : slug === "electricity-cost" ? "Electricity rate" : slug === "break-even" ? "Variable cost / unit" : "Amount / term"} value={c} setValue={setC} prefix={slug === "fuel-cost" || slug === "electricity-cost" || slug === "break-even" ? "$" : undefined} suffix={slug === "electricity-cost" ? "/kWh" : undefined}/>
          {slug === "tip-calculator" && <Field label="Number of people" value={d} setValue={setD}/>} 
          {slug === "loan-payment" || slug === "mortgage-payment" ? <p className="muted">For loans and mortgages, enter principal in Value, annual interest rate in Rate, and term in years in Amount / term.</p> : null}
        </>}
